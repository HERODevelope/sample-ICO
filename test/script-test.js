...
describe("ICO", function () {
    ...
    it("should allow investors to deposit funds", async function () {
        ...
        startTime = datetoTimestamp("2023/05/10 01:00:00"); 
    });

    it("should not allow deposits outside of ICO period--2", async function () {
        // Wait for ICO to end
        await ethers.provider.send("evm_setNextBlockTimestamp", [endTime + 1]);

        // Investor 1 tries to deposit 0.04 BNB
        await expect(ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") })).to.be.revertedWith("ICO has ended.");
    });
    ...
    it("should not allow deposits above hardcap", async function () {
        ...
        // Investor 2 tries to deposit 0.05 BNB
    ...
    it("should not allow investors to withdraw funds before ICO ended", async function () {
    ...
    it("should not allow investors to withdraw funds if no deposits to withdraw", async function () {
        // Investor 1 deposits 0.08 BNB
        await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
        await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
        ...
        // Softcap is not reached and Inestor 1 has no deposits
        await expect(ico.connect(investor2).withdraw()).to.be.revertedWith("No deposits to withdraw");
    });
    ...
    it("should allow investors to claim tokens after ICO if softcap is reached", async function () {
        ...
        expect(finalBalance1.sub(initialBalance1)).to.equal(80);
        expect(finalBalance2.sub(initialBalance2)).to.equal(20);
    });

    it("should not allow investors to claim tokens before ICO ended", async function () {
      // Investor 1 deposits 0.08 BNB
      await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
      await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
      ...
      // ICO has not ended, so investors cannot claim their tokens
      await expect(ico.connect(investor1).claim()).to.be.revertedWith("ICO has not ended or Hard cap not reached.");
    });
    ...
    it("should not allow investors to claim tokens after ICO if no deposits to withdraw", async function () {
        // Investor 1 deposits 0.08 BNB
        await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
        await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
        await ico.connect(investor1).deposit({ value: ethers.utils.parseEther("0.04") });
        ...
        // Softcap is not reached and Inestor 1 has no deposits
        await expect(ico.connect(investor2).claim()).to.be.revertedWith("No deposits to withdraw");
    });
